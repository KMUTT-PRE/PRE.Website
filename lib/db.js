...
function qualifyAppTables(sql) {
  ...
  const tableNames = ["news_posts", "news_images", "page_views", "session", "admin_users"];
...
async function initDb() {
  ...
  if (usePostgres) {
    ...
    const qualifiedAdminUsersTable = `${quoteIdentifier(postgresSchema)}.admin_users`;
    const sessionRegClassLiteral = qualifiedSessionTable.replaceAll("'", "''");
    ...
    await db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS ${qualifiedAdminUsersTable} (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_admin_users_username
        ON ${qualifiedAdminUsersTable}(username);
    `);

    // Legacy compatibility: older deployments created session.expire as INTEGER.
    // connect-pg-simple expects a timestamp column, so convert if needed.
    await db.exec(`
      DO $$
      DECLARE
        expire_type text;
      BEGIN
        SELECT atttypid::regtype::text
          INTO expire_type
          FROM pg_attribute
         WHERE attrelid = to_regclass('${sessionRegClassLiteral}')
           AND attname = 'expire'
           AND attnum > 0
           AND NOT attisdropped;

        IF expire_type IN ('integer', 'bigint') THEN
          EXECUTE 'ALTER TABLE ${qualifiedSessionTable} ALTER COLUMN expire TYPE TIMESTAMPTZ USING to_timestamp(expire::double precision / 1000.0)';
        END IF;
      END
      $$;
    `);
    ...
    if (postgresSchema !== "public") {
      ...
      try {
        await db.exec(`
          INSERT INTO ${qualifiedNewsPostsTable}
          ...
          INSERT INTO ${qualifiedNewsImagesTable}
          ...
          SELECT setval(
            pg_get_serial_sequence('${qualifiedNewsPostsTable.replaceAll("'", "''")}', 'id'),
            COALESCE((SELECT MAX(id) FROM ${qualifiedNewsPostsTable}), 1),
            true
          );

          SELECT setval(
            pg_get_serial_sequence('${qualifiedNewsImagesTable.replaceAll("'", "''")}', 'id'),
            COALESCE((SELECT MAX(id) FROM ${qualifiedNewsImagesTable}), 1),
            true
          );
        `);
      } catch (err) {
        console.warn(
          `[Database] Startup legacy schema migration skipped: ${err.message}`,
        );
      }
    }
